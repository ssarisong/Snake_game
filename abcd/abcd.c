...
void draw_map2(void) { //맵 테두리 그리는 함수 
    int i, j;
    for (i = 30; i < MAP_WIDTH; i++) {
        gotoxy(MAP_X + i, MAP_Y, "■");
    }
    for (i = MAP_Y + 1; i < MAP_Y + MAP_HEIGHT - 1; i++) {
        gotoxy(MAP_X, i, "■");
...
void move2(int dir) {
    ...
    if (x2[0] == food_x2 && y2[0] == food_y2) { //food와 충돌했을 경우 
        score2 += 10; //점수 증가 
        food2(); //새로운 food 추가 
        length2++; //길이증가 
...
void game_over(void) { //게임종료 함수 
    ...
    if (score > best_score) {
        best_score = score;
        gotoxy(MAP_X + (MAP_WIDTH / 2) - 4, MAP_Y + 10, "▲1player BEST SCORE▲");
    }
    else if (score2 > best_score2) {
        best_score2 = score2;
        gotoxy(MAP_X + (MAP_WIDTH / 2) - 4, MAP_Y + 11, "▲2player BEST SCORE▲");
    }
    Sleep(500);
    while (kbhit()) getch();
    key = getch();
...
void food2(void) {
    ...
    int r = 0; //난수 생성에 사동되는 변수 
    gotoxy(MAP_X, MAP_Y + MAP_HEIGHT+1, " 2player SCORE: "); //점수표시 
    printf("%3d, 2player SCORE: %3d, 2player BEST SCORE: %3d", score2, last_score2, best_score2);
